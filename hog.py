...
def fuzzy_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    total = player_score + take_turn(num_rolls, player_score, opponent_score, dice)
    return fuzzy_points(total)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while current_score<goal:
        ...
        while opponent_score<goal:
                if dice!= strategy(current_score,opponent_score):
                    return False
                opponent_score+=1
        current_score +=1
...
def make_averaged(original_function, total_samples=1000):
    ...
    #if *args {original_function}
    #iteration
    #iterate total_sample number of times
    #call og function
    #keep track of sum of result of og calls
    #find the mean
    return average
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while num_rolls <=10:
        #average_dice(num_rolls, dice)  
        averageturn = average_dice(num_rolls, dice)
        
        if averageturn > prevturn:
            prevturn=averageturn
...
def fuzzy_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    increase = fuzzy_update(0,score,opponent_score) - score
